...
const ContactModal = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // First, send the contact form email to admin
      ...
      if (response.status === 200) {
        // Then, send the auto-response email to the client
        const autoResponse = await emailjs.send(
          import.meta.env.VITE_EMAILJS_SERVICE_ID,
          import.meta.env.VITE_EMAILJS_AUTORESPONSE_TEMPLATE_ID,
          {
            to_name: formData.name,
            reply_to: formData.email,  // Important: This ensures the email goes to the client
            email: formData.email,     // Backup email field
            subject: `Thank you for contacting Namaste Sites | Ref: NS-${Date.now().toString(36)}`,
            message: formData.message,
